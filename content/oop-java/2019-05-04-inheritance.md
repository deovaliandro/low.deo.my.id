...
class turunan seperti `Ayah`, `Ibu` dan `Anak`. Ketika meng-extend sebuah class,
maka semua member non-private termasuk variabel dan method akan diwariskan ke
class tersebut.
...
2. Mendukung polimorphisme dengan adanya method overriding (dibahas pada bagian
   polimorphisme)
...
class John extends Person{
    John(String i, int j, double k, double l){
        super(i,j,k,l);
        super.myAge();
    }
...
class OverridingExample{
    public static void main(String args[]){
        John John = new John("John", 2002, 170.0, 50.0);
        System.out.println("My name is " + John.name);
        System.out.println("My ages is " + John.myAge());
    }
}
```
